...
fid_path = 'output/tt-9000.txt'
#'Python_Pascal_result_last_check.txt'
...
def input_reading_mod(input_dir, input_file):
    """This helper reads input from txt files"""
    # Check if input_file is a file and not a directory
    if not os.path.isfile(f'{input_dir}/{input_file}'):
        return None

    with open(f'{input_dir}/{input_file}', 'r') as input_fid:
        pred = input_fid.readlines()
    det = [x.strip('\n') for x in pred]
    return det
...
def gt_reading_mod(gt_dir, gt_id):
    ...
    file_path = f'{gt_dir}/poly_gt_{gt_id}.mat'
    if not os.path.isfile(file_path):
        return None

    gt = io.loadmat(file_path)
    gt = gt['polygt']
    return gt
...
for i, input_id in enumerate(tqdm(allInputs)):
    if (input_id != '.DS_Store'):
        ...
        if detections is None:
            print(f'Skipped directory {input_id}')
            continue
        
        groundtruths = gt_reading_mod(gt_dir, input_id)
        if groundtruths is None:
            print(f'Ground truth not found for {input_id}')
            continue
